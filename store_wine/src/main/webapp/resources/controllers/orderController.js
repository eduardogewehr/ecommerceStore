...
app.controller('OrderController', function($location, $scope, $http) {
	...
	$scope.getCurrentOrder = function() {
		$http({
			...
			async : true,
			cache : false,
			url : '/store_wine/rest/order/current'
	...
	$scope.getLastFinishOrder = function() {
		$http({
			...
			async : true,
			cache : false,
			url : '/store_wine/rest/order/last'
		}).then(function(response) {
	...
	$scope.addProductToOrder = function(item) {
		...
		$http({
			method : 'POST',
			data : item,
			url : '/store_wine/rest/order/item'
		}).then(function(response) {
	...
	$scope.calculateFreight = function(freight) {
		...
		$http({
			method : 'POST',
			data : freight,
			url : '/store_wine/rest/order/freight'
		}).then(function(response) {
	...
	$scope.finishOrder = function() {
		...
		$http({
			method : 'PUT',
			data : orderStatus,
			url : '/store_wine/rest/order/update'
		}).then(function(response) {
			$scope.createOrder();
			$location.path('/store_wine/confirmacao');
		}, function(error) {
	...
	$scope.deleteItemOrder = function(id) {
		...
		$http({
			method : 'DELETE',
			url : '/store_wine/rest/order/item/' + id
		}).then(function(response) {
